...
class SolicitacaoFormScreen extends StatefulWidget {
  final String? termo;
  const SolicitacaoFormScreen({super.key, this.termo});
...
class _SolicitacaoFormScreenState extends State<SolicitacaoFormScreen> {
  ...
  void initState() {
    super.initState();
    if ((widget.termo ?? '').isNotEmpty) _titulo.text = widget.termo!;
  }

  Future<void> _enviar() async {
    if (_titulo.text.isEmpty || _autor.text.isEmpty || _isbn.text.isEmpty || _ano.text.isEmpty || _editora.text.isEmpty) {
    ...
    try {
      final body = {
        ...
        'generos': _generos.text.split(',').map((s)=>s.trim()).where((s)=>s.isNotEmpty).toList(),
      };
  ...
  Widget _req(String label, TextEditingController c) => TextField(controller: c, decoration: InputDecoration(labelText: '$label *'));

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Solicitar cadastro de livro')),
      body: SingleChildScrollView(
        ...
        child: Column(children: [
          ...
          const SizedBox(height: 8),
          TextField(controller: _paginas, decoration: const InputDecoration(labelText: 'Nº de páginas')),
          const SizedBox(height: 8),
          TextField(controller: _capa, decoration: const InputDecoration(labelText: 'URL da Capa (opcional)')),
          const SizedBox(height: 8),
          TextField(controller: _generos, decoration: const InputDecoration(labelText: 'Gêneros (separados por vírgula)')),
          const SizedBox(height: 16),
          loading ? const CircularProgressIndicator() :
          ElevatedButton(onPressed: _enviar, child: const SizedBox(width: double.infinity, child: Center(child: Text('Enviar solicitação')))),
        ]),
      ),
    );
  }
